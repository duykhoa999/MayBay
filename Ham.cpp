	...
	do
	{
		...
		case F3:
		{
			...
			do
			{
				...
				{
					Red_Highlight();
					hienThongBao("Da huy tim kiem!");
					return tmp;
				}
				tontai = searchMB(lstMB, tmp.soHieuMayBay);
				if (tontai == -1) {
					Red_Highlight();
...
void QLMB(listMB& lstMB, PTRChuyenBay lstCB) {
	...
	while (exit1) {
		...
		switch (chonMB)
		{
			case THEM_MB:
			{
				mayBay mb = themMB(lstMB);
				if (mb.soHieuMayBay[0] == ESC)
				{
					Red_Highlight();
					hienThongBao("Da huy nhap!");
					break;
				}
				int themMB = insertMB(lstMB, mb);
				if (themMB == 1) {
					Green_Highlight();
...
int saveCB(PTRChuyenBay lstCB) {
	ChuyenBay cb;
	fstream outFile;
	outFile.open("D:/Study/CTDL/MayBay/DATA/ChuyenBay.txt", ios::out);
...
void QLCB(PTRChuyenBay& lstCB, listMB& lstMB) {
	...
	while (exit2)
	{
		...
		case HIEUCHINH_CB:
		{
			...
			{
				Red_Highlight();
				hienThongBao("Da co khach dat ve nen khong the hieu chinh");
			}
			else {
				Green_Highlight();
				hienThongBao("Da huy!");
		...
		case XOA_CB:
		{
			...
			if (demSoVe(p->data.dsVe, p->data.slVe) != 0) {
...
//ChonMayBay:
	do {
		...
		// xet gio lap chuyen bay 
		if (checkTime_LapCB(cb, lstCB) && _stricmp(cb.soHieuMayBay, mbCu.soHieuMayBay) != 0) {
			Red_Highlight();
			hienThongBao("May bay nay phai cat canh sau 5h nua! Hay chon may bay khac");
			//goto ChonMayBay;
...
void NhapCHUOI_SO(char var[], int len) {
	...
	while (true) {
		if (_kbhit()) {
			char c = _getch();
			if (!(!((int)c >= So0 && (int)c <= So9) && c != ' ') && nhap.n < maxInput) {
				if (nhap.n > 0 && *nhap.data[nhap.n - 1] == ' ' && c == ' ')
					continue;
				if (!(nhap.n == 0 && c == ' ')) {
					if (nhap.n == 0)
...
void xuatHK_1_CB(CHUYENBAY* cb, TREEHanhKhach lstHK, int page) {
	...
	for (j, pageHK; pageHK < cb->slVe; pageHK++, j++) {
		if (cb->dsVe[j] != "") {
			char CMND[21];
			strcpy(CMND, cb->dsVe[j].c_str());
			TREEHanhKhach t = timKiem_HK(lstHK, CMND);
...
void xuatALL_HK_1_CB(CHUYENBAY* cb, TREEHanhKhach lstHK) {
	Normal();
	int page = 0;
	if (cb->slVe < MAX_PAGE_SMALL)
		page = cb->slVe;
	int max_HK = demSoVe(cb->dsVe, cb->slVe);
	char kytu;
	bool max = false;
